...
export let vmLog = new Vue({
            ...
            if (newUser !== null) {
                ...
                if (this.type === 'follow') {
                    this.followInterval = setInterval(() => {
                        this.setTarget(this.followUser.user.pos.x, this.followUser.user.pos.y);
                    }, 100);
                } else if (this.type === 'around') {
                    let degree = 0;
                    this.followInterval = setInterval(() => {
                        let r = this.circleRadius;
                        let x0 = this.followUser.user.pos.x;
                        let y0 = this.followUser.user.pos.y;
                        degree += 10;
    ...
    methods: {
        ...
        toUserPos(user) {
            this.setTarget(user.pos.x, user.pos.y);
        },
        updateUsers(users) {
            this.users = users;
